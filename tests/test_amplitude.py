...
@pytest.mark.parametrize('shapeWaveform, messageWarningExpected', [
    ((2, 1000), 'all its values are zeros'),
])
def test_normalize_zero_waveform_emits_warning(shapeWaveform: tuple[int, int], messageWarningExpected: str) -> None:
    """Test that normalizeWaveform emits a warning and returns zeros for a silent waveform."""
    waveformZeros = numpy.zeros(shapeWaveform, dtype=numpy.float32)

    with pytest.warns(UserWarning, match=messageWarningExpected):
        waveformNormalized, revertNormalization = normalizeWaveform(waveformZeros)

    waveformReverted = revertNormalization(waveformNormalized)

    assert numpy.allclose(waveformNormalized, waveformZeros), (
        f"normalizeWaveform returned {waveformNormalized}, expected all zeros for {shapeWaveform=}."
    )
    assert numpy.allclose(waveformReverted, waveformZeros), (
        f"revertNormalization returned {waveformReverted}, expected all zeros for {shapeWaveform=}."
    )
